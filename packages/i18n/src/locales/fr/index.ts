...
"importConfirm": "Les extensions importées s’exécutent dans le processus de l’agent avec les mêmes droits que ses outils. Les compétences déclarées peuvent ajouter des instructions à l’agent et les dépendances déclarées sont installées avec npm (scripts d’installation désactivés). Continuer ?",
    ...
    "permissions": {
      ...
      "provider.request": "Envoyer des requêtes authentifiées à vos fournisseurs",
    ...
    "permissionHelp": {
      ...
      "provider.request": "Peut envoyer une requête authentifiée vers n'importe quel chemin d'un fournisseur que vous avez configuré, avec votre identifiant enregistré. Cela atteint des points de terminaison qui coûtent de l'argent ou lisent votre compte ; le plugin ne reçoit jamais la clé elle-même.",
