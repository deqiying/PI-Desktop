...
| `provider.request` | high | `ctx.providers.request` from a `contributes.agentExtensions` module: one authenticated HTTP request to a provider row the caller names, any path and any method (spec 16 §5.1) | Explicit confirmation, matching `agent.extension` | Reaches the whole provider API surface with the user's credential, including endpoints that spend money. The destination origin and the credential header stay host-owned (no origin change, no base-path escape, no redirect following, no header override), bodies and responses are capped, calls share the `agent.complete` brake at 8 per rolling 60 s and are capped at 4 in flight, and every call is audited as `provider.request` without a path, header value, field value, or credential (ADR 0301) |
